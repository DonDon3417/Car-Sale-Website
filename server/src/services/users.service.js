...
class UserService {
    ...
    async login(data) {
        ...
        if (user.isBlocked) {
            throw new BadRequestError('Tài khoản của bạn đã bị khóa. Vui lòng liên hệ admin.');
        }
    ...
    async getAllUser(query) {
        ...
        const users = await modelUser.find(filter).sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit));

        const total = await modelUser.countDocuments(filter);
